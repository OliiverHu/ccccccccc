...
import show_res.X_utils as X_utils
import time
...
def evaluate(model, 
    ...
    for i in range(generator.size()):
        ...
        for label in range(generator.num_classes()):
            all_annotations[i][label] = annotations[annotations[:, 4] == label, 0:4].copy()
...
def correct_yolo_boxes(boxes, image_h, image_w, net_h, net_w):
    ...
    for i in range(len(boxes)):
        try:
            ...
            # print("OverflowError!")
            # print("net_w=" + str(net_w))
            # print("net_h=" + str(net_h))
            ...
            # print("boxes[i].xmin" + str(boxes[i].xmin))
            # print("boxes[i].xmax" + str(boxes[i].xmax))
            # print("boxes[i].ymin" + str(boxes[i].ymin))
            # print("boxes[i].ymax" + str(boxes[i].ymax))
...
def decode_netout(netout, anchors, obj_thresh, net_h, net_w):
    ...
    netout[..., 5:7] = _sigmoid(netout[..., 5:7])
    netout[..., 7:]  = netout[..., 0][..., np.newaxis] * _softmax(netout[..., 7:])
    netout[..., 7:] *= netout[..., 7:] > obj_thresh
    ...
    for i in range(grid_h*grid_w):
        ...
        for b in range(nb_box):
            ...
            if w > 5 or h > 5 or x < 1e-5 or y < 1e-5:
                continue
...
def get_yolo_boxes(model, images, net_h, net_w, anchors, obj_thresh, nms_thresh, imagename=None):
    image_h, image_w, _ = images[0].shape
    nb_images           = len(images)
    batch_input         = np.zeros((nb_images, net_h, net_w, 5))
